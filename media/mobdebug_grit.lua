...
local snum = {} --FD = {[tostring(1/0)]='1/0 --[[math.huge]]',[tostring(-1/0)]='-1/0 --[[-math.huge]]',[tostring(0/0)]='0/0'}
...
local function s(t, opts)
  ...
  local function gensym(val) return '_'..(tostring(tostring(val)):gsub("[^[0-9a-zA-Z]]",""):gsub("([0-9][0-9a-zA-Z]+)",
  ...
  local function safename(path, name) -- generates foo.bar, foo[3], or foo['b a r']
    ...
    local plain = type(n) == "string" and n:match("^[[a-z][A-Z]_][[0-9a-zA-Z]_]*$") and not keyword[n]
  ...
  local alphanumsort = type(opts.sortkeys) == 'function' and opts.sortkeys or function(k, o, n) -- k=keys, o=originaltable, n=padding
    ...
    table.sort(k, function(a,b)
      ...
      return (k[a] ~= nil and 0 or to[type(a)] or 'z')..(tostring(a):gsub("[0-9]+",padnum))
           < (k[b] ~= nil and 0 or to[type(b)] or 'z')..(tostring(b):gsub("[0-9]+",padnum)) end) end
...
local function is_pending(peer,nb)
  ...
  if not buf and checkcount >= mobdebug.checkcount then
    ...
    buf = peer:receive("is_pending"..nb,1)
...
local function readnext(peer, num)
  peer:settimeout(0) -- non-blocking
  local res, err, partial = peer:receive("readnext",num)
  peer:settimeout() -- back to blocking
...
local function handle_breakpoint(peer)
 ...
 -- if buf then
 --   print("handle_breakpoint="..buf)
 -- else
 --   print("handle_breakpoint=nil")
 -- end
  if buf then
 -- print("handle_breakpoint(peer) buf="..buf)
  end
 ...
 --   print("handle_breakpoint=nil")
  ...
  if buf ~= 'SETB ' and buf ~= 'DELB ' then print("not SETB or DELB"..buf); return end

  local res, _, partial = peer:receive("handle_breakpoint") -- get the rest of the line; blocking
  if not res then
    if partial then buf = buf .. partial end
    return
  end

  local _, _, cmd, file, line = (buf..res):find("^([A-Z]+)[ ]+(.+)[ ]+([0-9]+)[ ]*$")
...
--        file = string.gsub(file, "\\", "/")
    ...
    if is_pending(server,"y") then handle_breakpoint(server) end
    ...
    elseif not status and res then
      --error(tostring(res), 2) -- report any other (internal) errors back to the application
    end
...
local function isrunning()
  if coro_debugger then
     print("isrunning() corostatus(coro_debugger)="..corostatus(coro_debugger))
  else
     print("isrunning() coro_debugger=nil")
  end
...
--  print("debugger_loop()")
  while true do
   ...
   -- print(server)
   -- print(wx)
  --  print(mobdebug.yield)
  --  print(server.settimeout)

    if (wx or mobdebug.yield) and server.settimeout then 
      server:settimeout(mobdebug.yieldtimeout)
  --    print("mobdebug.yieldtimeout="..mobdebug.yieldtimeout)
    end
  --  print(server.timeout)
   -- print("server.timeout="..server.timeout)
    while true do
      line, err = server:receive("receive from debugger_loop() while")
     -- if err == true then
   ...
   --   else
    ...
    if command == "SETB" then
      local _, _, _, file, line = string.find(line, "^([A-Z]+)[ ]+(.+)[ ]+([0-9]+)[ ]*$")
      if file and line then
        set_breakpoint(file, tonumber(line))
        server:send("200 OK\n")
      else
        server:send("400 Bad Request 1\n")
      end
    elseif command == "DELB" then
      -- line = "DELB * 0\n"
      -- s, e, c, file, line = string.find(line, "^([A-Z]+)[ ]+(.+)[ ]+([0-9]+)[ ]*$")
      local _, _, _, file, line = string.find(line, "^([A-Z]+)[ ]+(.+)[ ]+([0-9]+)[ ]*$")
      if file and line then
        remove_breakpoint(file, tonumber(line))
        server:send("200 OK\n")
      else
        server:send("400 Bad Request 2\n")
      end
    elseif command == "EXEC" then
      -- extract any optional parameters
      print("EXEC")
      print(line)
      local params = "" --FD string.match(line, "--[ ]*(%b{})[ ]*$")
      local _, _, chunk = string.find(line, "^[A-Z]+[ ]+(.+)$")
      if chunk then
        print(chunk)
        ...
        server:send("400 Bad Request 3\n")
      end
    elseif command == "LOAD" then
      print("command load line="..line)
      local s, e, size, name = string.find(line, "^[A-Z]+[ ]+([0-9]+)[ ]+(.+)[ \n]*$")
      print("s="..s.." e="..e.." size="..size)
      print("name="..name)
      print("--end--")
      size = tonumber(size)
      ...
      if abort == nil then -- no LOAD/RELOAD allowed inside start()
        ...
        if size > 0 then 
           ...
           local buf, err server:receive("receive from LOAD",size)
           print(buf)
           print(err)
        end
        ...
        if size == 0 and name == '-' then -- RELOAD the current script being debugged
          ...
          local chunk = size == 0 and "" or server:receive("receive from else LOAD",size)
          if chunk then -- LOAD a new script for debugging
            ...
            server:send("400 Bad Request 4\n")
          end
        end
      end

    elseif command == "SETW" then
      local _, _, exp = string.find(line, "^[A-Z]+[ ]+(.+)[ ]*$")
      if exp then
        ...
        server:send("400 Bad Request 5\n")
      end
    elseif command == "DELW" then
      local _, _, index = string.find(line, "^[A-Z]+[ ]+([0-9]+)[ ]*$")
    ...
    elseif command == "BASEDIR" then
      local s, e, dir = string.find(line, "^[A-Z]+[ ]+(.+)[ ]*$")
      print("s=".." e="..e)
      if dir then
    ...
    elseif command == "OUTPUT" then
      local _, _, stream, mode = string.find(line, "^[A-Z]+[ ]+([0-9a-zA-Z]+)[ ]+([dcr])[ ]*$")
      if stream and mode and stream == "stdout" then
        ...
        server:send("400 Bad Request 8\n")
      end
    elseif command == "EXIT" then
      server:send("200 OK\n")
      coroyield("exit")
    else
      server:send("400 Bad Request 9\n")
    end
  end
end
...
local function start(controller_host, controller_port)
  ...
  if isrunning() then print("already running") return end
  ...
  if server then
  print("mobdebug.start() connected")
    ...
    if coro_debugger then
    print("mobdebug.start() coro_debugger="..tostring(coro_debugger))
    else
    print("mobdebug.start() coro_debugger=nil")
    end
...
local function on()
  if not isrunning() then print("on() not runnning") end
  if not server then print("on() no server") end
  if not (isrunning() and server) then return end
  print("on")
...
local function handle(params, client, options)
  ...
  or command == "over" or command == "exit" then
    ...
    client:receive("receive from handle 1") -- this should consume the first '200 OK' response
    while true do
      local done = true
      local breakpoint = client:receive("receive from handle 2")
      if not breakpoint then
        print("Program finished")
        return nil, nil, false
      end
      local _, _, status = string.find(breakpoint, "^([0-9]+)")
      if status == "200" then
        -- don't need to do anything
      elseif status == "202" then
        _, _, file, line = string.find(breakpoint, "^202 Paused[ ]+(.+)[ ]+([0-9]+)[ ]*$")
        if file and line then
          print("Paused at file " .. file .. " line " .. line)
        end
      elseif status == "203" then
        _, _, file, line, watch_idx = string.find(breakpoint, "^203 Paused[ ]+(.+)[ ]+([0-9]+)[ ]+([0-9]+)[ ]*$")
        if file and line and watch_idx then
          print("Paused at file " .. file .. " line " .. line .. " (watch expression " .. watch_idx .. ": [" .. watches[watch_idx] .. "])")
        end
      elseif status == "204" then
        local _, _, stream, size = string.find(breakpoint, "^204 Output ([0-9a-zA-Z]+) ([0-9]+)$")
        if stream and size then
          local size = tonumber(size)
          local msg = size > 0 and client:receive("receive from handle 3",size) or ""
          print(msg)
      ...
      elseif status == "401" then
        local _, _, size = string.find(breakpoint, "^401 Error in Execution ([0-9]+)$")
        if size then
          local msg = client:receive("receive from handle 4",tonumber(size))
  ...
  elseif command == "setb" or command == "asetb" then
    _, _, _, file, line = string.find(params, "^([a-z]+)[ ]+(.+)[ ]+([0-9]+)[ ]*$")
    if file and line then
      ...
      if command == "asetb" or client:receive("receive from handle 5") == "200 OK" then
  ...
  elseif command == "setw" then
    local _, _, exp = string.find(params, "^[a-z]+[ ]+(.+)$")
    if exp then
      client:send("SETW " .. exp .. "\n")
      local answer = client:receive("receive from handle 6")
      local _, _, watch_idx = string.find(answer, "^200 OK ([0-9]+)[ ]*$")
      if watch_idx then
        watches[watch_idx] = exp
        print("Inserted watch exp no. " .. watch_idx)
      else
        local _, _, size = string.find(answer, "^401 Error in Expression ([0-9]+)$")
        if size then
          local err = client:receive("receive from handle 7",tonumber(size)):gsub(".+:[0-9]+:[ ]*","")
  ...
  elseif command == "delb" or command == "adelb" then
    ...
    if file and line then
      ...
      if command == "adelb" or client:receive("receive from handle 8") == "200 OK" then
  ...
  elseif command == "delallb" then
    ...
    if client:receive("receive from handle 9") == "200 OK" then
      remove_breakpoint(file, line)
    else
      print("Error: all breakpoints not removed")
    end
  elseif command == "delw" then
    local _, _, index = string.find(params, "^[a-z]+[ ]+([0-9]+)[ ]*$")
    if index then
      client:send("DELW " .. index .. "\n")
      if client:receive("receive from handle 10") == "200 OK" then
        watches[index] = nil
      else
  ...
  elseif command == "delallw" then
    for index, exp in pairs(watches) do
      client:send("DELW " .. index .. "\n")
      if client:receive("receive from handle 11") == "200 OK" then
        watches[index] = nil
      else
  ...
  elseif command == "eval" or command == "exec"
    ...
    if exp or (command == "reload") then
      ...
      elseif command == "loadstring" then
        local _, _, _, file, lines = string.find(exp, "^([\"'])(.+)%1[ ](.+)")
        if not file then
           _, _, file, lines = string.find(exp, "^([ ]+)[ ](.+)")
        end
      ...
      while true do
        local params, err = client:receive("receive from handle 12")
        if not params then
          return nil, nil, "Debugger connection " .. (err or "error")
        end
        local done = true
        local _, _, status, len = string.find(params, "^([0-9]+).+[ ]+([0-9]+)[ ]*$")
        if status == "200" then
          len = tonumber(len)
          if len > 0 then
            local status, res
            local str = client:receive("receive from handle 13",len)
        ...
        elseif status == "201" then
          _, _, file, line = string.find(params, "^201 Started[ ]+(.+)[ ]+([0-9]+)[ ]*$")
        ...
        elseif status == "204" then
          local _, _, stream, size = string.find(params, "^204 Output ([0-9a-zA-Z]+) ([0-9]+)$")
          if stream and size then
            local size = tonumber(size)
            local msg = size > 0 and client:receive("receive from handle 14",size) or ""
            print(msg)
            if outputs[stream] then outputs[stream](msg) end
            -- this was just the output, so go back reading the response
            done = false
        ...
        elseif status == "401" then
          len = tonumber(len)
          local res = client:receive("receive from handle 15",len)
  ...
  elseif command == "suspend" then
    client:send("SUSPEND\n")
  elseif command == "stack" then
    local opts = string.match(params, "^[a-z]+[ ]+(.+)$")
    client:send("STACK" .. (opts and " "..opts or "") .."\n")
    local resp = client:receive("receive from handle 16")
    local _, _, status, res = string.find(resp, "^([0-9]+)[ ]+[0-9a-zA-Z]+[ ]+(.+)[ ]*$")
    if status == "200" then
      ...
      return stack
    elseif status == "401" then
      local _, _, len = string.find(resp, "[ ]+([0-9]+)[ ]*$")
      len = tonumber(len)
      local res = len > 0 and client:receive("receive from handle 17",len) or "Invalid stack information."
      print("Error in expression: " .. res)
      return nil, nil, res
    else
      print("Unknown error")
  ...
  elseif command == "output" then
    local _, _, stream, mode = string.find(params, "^[a-z]+[ ]+([0-9a-zA-Z]+)[ ]+([dcr])[ ]*$")
    if stream and mode then
      client:send("OUTPUT "..stream.." "..mode.."\n")
      local resp, err = client:receive("receive from handle 18")
      if not resp then
        print("Unknown error: "..err)
        return nil, nil, "Debugger connection error: "..err
      end
      local _, _, status = string.find(resp, "^([0-9]+)[ ]+[0-9a-zA-Z]+[ ]*$")
      if status == "200" then
  ...
  elseif command == "basedir" then
    local _, _, dir = string.find(params, "^[a-z]+[ ]+(.+)$")
    if dir then
      ...
      local resp, err = client:receive("receive from handle 19")
      if not resp then
        print("Unknown error: "..err)
        return nil, nil, "Debugger connection error: "..err
      end
  ...
  elseif command == "help" then
    ...
    local _, _, spaces = string.find(params, "^([ ]*)$")
    if spaces then
...
local function listen(host, port)
  ...
  client:send("STEP\n")
  client:receive("receive from listen 1")

  local breakpoint = client:receive("receive from listen 2")
  local _, _, file, line = string.find(breakpoint, "^202 Paused[ ]+(.+)[ ]+([0-9]+)[ ]*$")
  if file and line then
    ...
    if size then
      ...
      print(client:receive("receive from listen 3",size))
    end
  end
